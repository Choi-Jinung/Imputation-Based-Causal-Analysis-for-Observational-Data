...
var_est<- function(control, treatment, IV, M_B, para_curr, EM, covariate, B, seed) {
  ...
  # calculate_95_CIs <- function(boot_iterations_out, control, treatment, IV, M_B, para_curr, EM, covariate, seed) {
  #   
  #   # PSM
  #   control_psm <- data.frame(control) %>% rename(y=y0) %>% mutate(t=0)
  #   treatment_psm <- data.frame(treatment) %>% rename(y=y1) %>% mutate(t=1)
  ...
  # 
  #   # CIA
  #   reg_ATT_list <- Regression_CI(control, treatment, covariate)
  #   reg_ATC_list <- Regression_CI(treatment, control, covariate)
  #   reg_ATT <- reg_ATT_list$observed_bar - reg_ATT_list$missing_bar
  #   reg_ATC <- reg_ATC_list$missing_bar - reg_ATC_list$observed_bar
  ...
  #   # FI
  #   set.seed(1)
  #   impute_control <- FI_single(dataA=treatment, dataB=control, IV=IV, M_B=M_B, para_curr=para_curr, EM=T)
  #   impute_treatment <- FI_single(dataA=control, dataB=treatment, IV=IV, M_B=M_B, para_curr=para_curr, EM=T)
  ...
  #   FI_ATE_estimates <- c(FI_ATT, FI_ATC, FI_ATE)
  #   
  #   p_values_PSM <- (1-pnorm(q=PSM_ATE_estimates, mean=0, sd=sqrt(boot_iterations_out$var_ATE_estimates_PSM)))*2
  #   p_values_reg <- (1-pnorm(q=reg_ATE_estimates, mean=0, sd=sqrt(boot_iterations_out$var_ATE_estimates_reg)))*2
  #   p_values_FI <- (1-pnorm(q=FI_ATE_estimates, mean=0, sd=sqrt(boot_iterations_out$var_ATE_estimates_FI)))*2
  #   
  #   point_estimates <- list(ATE_reg=reg_ATE_estimates, 
  #                           ATE_FI=FI_ATE_estimates, 
  #                           PSM_ATE_estimates=PSM_ATE_estimates)
  #   p_values <- list(p_values_reg=p_values_reg, 
  #                    p_values_FI=p_values_FI, 
  #                    p_values_PSM=p_values_PSM)
  #   
  #   end <- proc.time() - start
  #   
  #   calculate_95_CIs_out <- list(point_estimates=point_estimates, p_values=p_values, cal_time=end)
  #   return(calculate_95_CIs_out)
  # }
  # causal_inference_out <- calculate_95_CIs(boot_iterations_out, control, treatment, IV, M_B, para_curr, EM, covariate, seed)
  # return(causal_inference_out)
  return(boot_iterations_out)
}
